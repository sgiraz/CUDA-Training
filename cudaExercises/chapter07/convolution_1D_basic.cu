...
void sequentialConv(float *h_N, float *h_M, float *h_PS, int n, int sizeMask){
...
int main(void) {
    ...
    msTime = convolution_1D_basic(h_N, h_M, h_P, sizeMask, n);
    
    // -------------------------- perform sequential convolution --------------------------
    cudaEventRecord(startTimeCuda, 0);
    sequentialConv(h_N, h_M, h_PS, n, sizeMask);
    cudaEventRecord(stopTimeCuda, 0);
    cudaEventSynchronize(stopTimeCuda);
